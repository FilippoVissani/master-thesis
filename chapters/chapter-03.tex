...
\begin{itemize}
    ...
    \item \textbf{Currying}: Currying is the process of converting a function with multiple arguments into a sequence of functions that take one argument. Each function returns another function that consumes the following argument.
\end{itemize}
...
Even before addressing the problems presented in the Section, it is necessary to understand whether it is possible to create a Kotlin version of FRASP and, if so, analyze the similarities and differences with the model implemented in Scala, considering, in particular, the ergonomics of the \ac{dsl}.

\footnote{\url{https://github.com/FilippoVissani/kotlin-distributed-frp}}
...
\lstinputlisting[float,label={lst:kotlin-distributed-frp},language=kotlin,caption=Gradient implementation in Kotlin Distributed FRP]{listings/kotlin-distributed-frp.kt}
