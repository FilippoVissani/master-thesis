...
In the \Cref{lst:collektive-gradient}, the implementation of the gradient in Collektive is presented. In this case, it is used the \texttt{share} construct~\cite{https://doi.org/10.48550/arxiv.1910.02874}. This construct is not part of field calculus but extends it; it allows declarative
expression of unified and coherent operation mechanisms for state preservation, communication to neighbors, and aggregation of received messages. In~\cite{https://doi.org/10.48550/arxiv.1910.02874} is demonstrated that \texttt{share} can be used to replace the combination of \texttt{rep} and \texttt{nbr}. \texttt{share} captures the space-time nature of field computation through observation of neighbors' values, starting from an \texttt{initial} value, it reduces to a single local value given a \texttt{transform} function and updating and sharing to neighbors of a local variable.
